...
import {getFinancialHighlights, getListOfFinancialHighlights} from '@/agents/repository/Fialda'
...
export class VNSCStockFundamental extends Tool {
  ...
  constructor() {
    ...
    this.description = `Đây là công cụ lấy thông tin các chỉ số tài chình cơ bản của một mã chứng khoán, có thể dùng để đưa ra nhận định về tình hình kinh doanh, làm ăn và sức khỏe tài chính của doanh nghiệp. Tham số truyền vào là mã chứng khoán hoặc một danh sách mã chứng khoán phân cách bằng dấu ','.`;
  }
  ...
  async _call(arg: string): Promise<string> {
    try {
      ...
      if (symbol_list.length > 0) {
        const hightlights = await getListOfFinancialHighlights(symbol_list);
        return `
